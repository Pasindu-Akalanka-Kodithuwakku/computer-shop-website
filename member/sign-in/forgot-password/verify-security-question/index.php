...
/* check mobile number */
    ...
    $sql = "SELECT * FROM `Member` WHERE `contact` = '$tel' AND `trash` = 0 LIMIT 1";
    $stmt = mysqli_query($conn, $sql);
    if ($stmt == true && mysqli_num_rows($stmt) == 1) {
        $row = mysqli_fetch_assoc($stmt);
        $tel = $row['contact'];
    } else {
        header("location: /member/sign-in/forgot-password/?error=Incorrect mobile number."); # redirect to back page
        exit();
    }
/* check errors */
    if (isset($_GET['error'])) {
...
<body>
    ...
    <!-- sign in -->
        <div class="box-layout">
            <h3 class="title-header">Verify security question</h3>
