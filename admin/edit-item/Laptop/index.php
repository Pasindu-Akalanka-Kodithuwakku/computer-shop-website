...
/* check is valid product id */
    ...
    if (!(isset($_REQUEST['category']) && strtoupper($_REQUEST['category']) == "LAPTOP")) {
...
/* set variables */
    ...
    $txtGraphic = '';
    $selScreenSize = $specification[8];
    $txtScreenSize = '';
    $selResolution = $specification[9];
    $txtResolution = '';
    $selOperatingSystem = $specification[10];
    $txtOperatingSystem = '';
...
/* click submit button */
    ...
    /* set new values */
        ...
        $selOperatingSystem = $_POST['selOperatingSystem'];
        $txtOperatingSystem = trim($_POST['txtOperatingSystem']);
    ...
    /* check errors */
        if ($selCapacity == "none" && strlen($txtCapacity) < 1) { $info[] = "Laptop RAM capacity (size) field is required. ⁉️"; /* capacity field not fill error*/ }
        if ($selMemoryType == "none" && strlen($txtMemoryType) < 1) { $info[] = "Laptop memory type field is required. ⁉️"; /* memory type field not fill error */ }
        if ($selProcessorGeneration == "none" && strlen($txtProcessorGeneration) < 1) { $info[] = "Laptop processor generation field is required. ⁉️"; /* processor generation field not fill error */ }
        if ($selProcessorModel == "none" && strlen($txtProcessorModel) < 1) { $info[] = "Laptop processor model field is required. ⁉️"; /* processor model field not fill error */ }
        if ($selProcessorSeries == "none" && strlen($txtProcessorSeries) < 1) { $info[] = "Laptop processor series field is required. ⁉️"; /* processor series field not fill error */ }
        if ($selHardDrive == "none" && strlen($txtHardDrive) < 1) { $info[] = "Laptop hard drive field is required. ⁉️"; /* hard drive field not fill error */ }
        if ($selHardDriveCapacity == "none" && strlen($txtHardDriveCapacity) < 1) { $info[] = "Laptop hard drive capacity field is required. ⁉️"; /* hard drive capacity field not fill error */ }
        if ($selGraphic == "none" && strlen($txtGraphic) < 1) { $info[] = "Laptop graphic field is required. ⁉️"; /* graphic field not fill error */ }
        if ($selScreenSize == "none" && strlen($txtScreenSize) < 1) { $info[] = "Laptop screen size field is required. ⁉️"; /* screen size field not fill error */ }
        if ($selResolution == "none" && strlen($txtResolution) < 1) { $info[] = "Laptop resolution field is required. ⁉️"; /* resolution field not fill error */ }
        if ($selOperatingSystem == "none" && strlen($txtOperatingSystem) < 1) { $info[] = "Laptop operating system field is required. ⁉️"; /* operating system field not fill error */ }
    ...
    /* no errors */
        if (empty($info)) {
            ...
            if ($txtOperatingSystem > 0) { $selOperatingSystem = "none"; $operatingSystem = mysqli_real_escape_string($conn, $txtOperatingSystem); } else { $operatingSystem = mysqli_real_escape_string($conn, $selOperatingSystem); }
        /* join specification */
            $specification = array($capacity, $memoryType, $processorGeneration, $processorModel, $processorSeries, $hardDrive, $hardDriveCapacity, $graphic, $screenSize, $resolution, $operatingSystem);
...
<body>
    ...
    <!-- add new items -->
        <div class="box-layout">
            ...
            <form action="/admin/edit-item/Laptop/" method="post" enctype="multipart/form-data" autocomplete="off">
                <?php
            ...
            <!-- capacity -->
                ...
                <p class="custom-note">The laptop RAM capacities you have previously added are listed here,
                    and if the laptop RAM capacity you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop RAM capacity in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop RAM capacity,
                    it will be considered the laptop RAM capacity of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selCapacity">
                        ...
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'Laptop' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                    ...
                    <p>Or</p>
                <!-- enter new capacity -->
                    <input type="text" name="txtCapacity" placeholder="Enter new RAM capacity (Ex: Capacity: 32GB)" value="<?php echo $txtCapacity; ?>" maxlength="30">
                </fieldset>
                <p class="custom-note">Please enter 1 to 30 characters</p>
            <!-- memory type -->
                <h3 class="sub-header">Memory Type *</h3>
                <p class="custom-note">The laptop memory types you have previously added are listed here,
                    and if the laptop memory type you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop memory type in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop memory type,
                    it will be considered the laptop memory type of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- processor generation -->
                ...
                <p class="custom-note">The laptop processor generations you have previously added are listed here,
                    and if the laptop processor generation you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop processor generation in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop processor generation,
                    it will be considered the laptop processor generation of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- processor model -->
                ...
                <p class="custom-note">The laptop processor models you have previously added are listed here,
                    and if the laptop processor model you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop processor model in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop processor model,
                    it will be considered the laptop processor model of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- processor series -->
                ...
                <p class="custom-note">The laptop processor series you have previously added are listed here,
                    and if the laptop processor series you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop processor series in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop processor series,
                    it will be considered the laptop processor series of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- hard drive -->
                ...
                <p class="custom-note">The laptop hard drives you have previously added are listed here,
                    and if the laptop hard drive you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop hard drive in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop hard drive,
                    it will be considered the laptop hard drive of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- hard drive capacity -->
                ...
                <p class="custom-note">The laptop hard drive capacities you have previously added are listed here,
                    and if the laptop hard drive capacity you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop hard drive capacity in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop hard drive capacity,
                    it will be considered the laptop hard drive capacity of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                ...
                <!-- enter new hard drive capacity -->
                    <input type="text" name="txtHardDriveCapacity" placeholder="Enter new hard drive capacity (Ex: Drive Capacity: 256GB)" value="<?php echo $txtHardDriveCapacity; ?>" maxlength="30">
                </fieldset>
                <p class="custom-note">Please enter 1 to 30 characters</p>
            <!-- graphics -->
                <h3 class="sub-header">Graphics *</h3>
                <p class="custom-note">The laptop graphics you have previously added are listed here,
                    and if the laptop graphic you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop graphic in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop graphic,
                    it will be considered the laptop graphic of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                ...
                <p class="custom-note">Please enter 1 to 50 characters</p>
            <!-- screen size -->
                <h3 class="sub-header">Screen Size *</h3>
                <p class="custom-note">The laptop screen sizes you have previously added are listed here,
                    and if the laptop screen size you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop screen size in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop screen size,
                    it will be considered the laptop screen size of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
            ...
            <!-- resolution -->
                ...
                <p class="custom-note">The laptop resolutions you have previously added are listed here,
                    and if the laptop resolution you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop resolution in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop resolution,
                    it will be considered the laptop resolution of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selResolution">
                        <option value="none">-- Select a previously entered name --</option>
                        <option value="Resolution: Full HD 1920 x 1080" <?php echo $selResolution == "Resolution: Full HD 1920 x 1080" ? "selected" : '' ; ?>>Full HD 1920 x 1080</option>
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'Laptop' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                            $savedRecords = array("Resolution: Full HD 1920 x 1080");
                            $foundRecord = 0;
                            if ($stmt == true) {
                                while ($row = mysqli_fetch_assoc($stmt)) {
                                    foreach (explode(", ", $row['specification']) as $key => $value) {
                ...
                <!-- enter new resolution -->
                    <input type="text" name="txtResolution" placeholder="Enter new resolution (Ex: Resolution: Full HD 1920 x 1080)" value="<?php echo $txtResolution; ?>" maxlength="50">
                </fieldset>
                <p class="custom-note">Please enter 1 to 50 characters</p>
            <!-- operating system -->
                <h3 class="sub-header">Operating System *</h3>
                <p class="custom-note">The laptop operating systems you have previously added are listed here,
                    and if the laptop operating system you are adding appears in the list, you can select it. Alternatively, you can enter your new laptop operating system in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the laptop operating system,
                    it will be considered the laptop operating system of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selOperatingSystem">
                        <option value="none">-- Select a previously entered name --</option>
                        <option value="Operating System: Windows 10" <?php echo $selOperatingSystem == "Operating System: Windows 10" ? "selected" : '' ; ?>>Windows 10</option>
                        <option value="Operating System: Windows 11" <?php echo $selOperatingSystem == "Operating System: Windows 11" ? "selected" : '' ; ?>>Windows 11</option>
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'Laptop' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                            $savedRecords = array("Operating System: Windows 10", "Operating System: Windows 11");
                            $foundRecord = 0;
                            if ($stmt == true) {
                                while ($row = mysqli_fetch_assoc($stmt)) {
                                    foreach (explode(", ", $row['specification']) as $key => $value) {
                                        if ($key == 10) {
                                            ...
                                            if ($foundRecord == 0) {
                                                ?><option value="<?php echo $value; ?>" <?php echo $selOperatingSystem == $value ? "selected" : '' ; ?>>
                                                    <?php echo $value; ?>
                                                </option><?php
                                            }
                                            $foundRecord = 0;
                ...
                <!-- enter new operating system -->
                    <input type="text" name="txtOperatingSystem" placeholder="Enter new operating system (Ex: Operating System: Windows 10 Home Single Language)" value="<?php echo $txtOperatingSystem; ?>" maxlength="50">
                </fieldset>
                <p class="custom-note">Please enter 1 to 50 characters</p>
            <!-- description -->
