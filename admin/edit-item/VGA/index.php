...
/* check is valid product id */
    ...
    if (!(isset($_REQUEST['category']) && strtoupper($_REQUEST['category']) == "VGA")) {
...
<body>
    ...
    <!-- add new items -->
        <div class="box-layout">
            ...
            <form action="/admin/edit-item/VGA/" method="post" enctype="multipart/form-data" autocomplete="off">
                <?php
            ...
            <!-- capacity -->
                <h3 class="sub-header">Graphic Card Capacity (Size) *</h3>
                <p class="custom-note">The VGA sizes you have previously added are listed here,
                    and if the VGA size you are adding appears in the list, you can select it. Alternatively, you can enter your new VGA size in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the VGA size,
                    it will be considered the VGA size of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selCapacity">
                        ...
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'VGA' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                            $savedRecords = array("Capacity: 4GB", "Capacity: 8GB");
                            $foundRecord = 0;
                            if ($stmt == true) {
                    ...
                    <p>Or</p>
                <!-- enter new capacity -->
                    <input type="text" name="txtCapacity" placeholder="Enter new VGA capacity (Ex: Capacity: 12GB)" value="<?php echo $txtCapacity; ?>" maxlength="20">
                </fieldset>
                <p class="custom-note">Please enter 1 to 20 characters</p>
            <!-- description -->
                <h3 class="sub-header">Description (Optional)</h3>
                <textarea name="rtxtDescription" placeholder="Enter description (Ex: Cooling Fans: 2 RGB, etc..)" maxlength="1500"><?php echo $rtxtDescription; ?></textarea>
