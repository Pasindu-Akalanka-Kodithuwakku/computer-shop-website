...
/* check is valid product id */
    ...
    if (!(isset($_REQUEST['category']) && strtoupper($_REQUEST['category']) == "POWER SUPPLY")) {
...
/* set variables */
    ...
    $selPowerSupplyWatt = $specification[0];
    $txtPowerSupplyWatt = '';
...
/* click submit button */
    if (!empty($_POST)) {
    /* set new values */
        $selPowerSupplyWatt = $_POST['selPowerSupplyWatt'];
        $txtPowerSupplyWatt = trim($_POST['txtPowerSupplyWatt']);
    ...
    /* check errors */
        if ($selPowerSupplyWatt == "none" && strlen($txtPowerSupplyWatt) < 1) {
            $info[] = "Power supply watt field is required. ⁉️"; # supply watt field not fill error
        }
    ...
    /* no errors */
        if (empty($info)) {
            if ($txtPowerSupplyWatt > 0) {
                $selPowerSupplyWatt = "none"; # reset capacity list
                $powerSupplyWatt = mysqli_real_escape_string($conn, $txtPowerSupplyWatt);
            } else {
                $powerSupplyWatt = mysqli_real_escape_string($conn, $selPowerSupplyWatt);
            }
        /* join specification */
            $specification = array($powerSupplyWatt);
...
<body>
    ...
    <!-- add new items -->
        <div class="box-layout">
            ...
            <form action="/admin/edit-item/PowerSupply/" method="post" enctype="multipart/form-data" autocomplete="off">
                <?php
            ...
            <!-- supply waat -->
                <h3 class="sub-header">Power Supply Watt *</h3>
                <p class="custom-note">The Power Supply Watt values you have previously added are listed here,
                    and if the Power Supply Watt value you are adding appears in the list, you can select it. Alternatively, you can enter your new Power Supply Watt value in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the Power Supply Watt value,
                    it will be considered the Power Supply Watt value of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selPowerSupplyWatt">
                        <option value="none">-- Select a previously entered name --</option>
                        <option value="Waat: 500W" <?php echo $selPowerSupplyWatt == "Waat: 500W" ? "selected" : '' ; ?>>500W</option>
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'Power Supply' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                            $savedRecords = array("Waat: 500W");
                            $foundRecord = 0;
                            if ($stmt == true) {
                                while ($row = mysqli_fetch_assoc($stmt)) {
                                    foreach (explode(", ", $row['specification']) as $key => $value) {
                                        if ($key == 0) {
                                            ...
                                            if ($foundRecord == 0) {
                                                ?><option value="<?php echo $value; ?>" <?php echo $selPowerSupplyWatt == $value ? "selected" : '' ; ?>>
                                                    <?php echo $value; ?>
                                                </option><?php
                                            }
                                            $foundRecord = 0;
                    ...
                    <p>Or</p>
                <!-- enter new supply watt value -->
                    <input type="text" name="txtPowerSupplyWatt" placeholder="Enter new power supply watt value (Ex: Waat: 700W)" value="<?php echo $txtPowerSupplyWatt; ?>" maxlength="15">
                </fieldset>
                <p class="custom-note">Please enter 1 to 15 characters</p>
            <!-- description -->
                <h3 class="sub-header">Description (Optional)</h3>
                <textarea name="rtxtDescription" placeholder="Enter description (Ex: Input Voltage: 230V, etc..)" maxlength="1500"><?php echo $rtxtDescription; ?></textarea>
