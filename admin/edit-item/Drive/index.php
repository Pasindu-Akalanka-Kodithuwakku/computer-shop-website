...
/* check is valid product id */
    ...
    if (!(isset($_REQUEST['category']) && (strtoupper($_REQUEST['category']) == "HDD" || strtoupper($_REQUEST['category']) == "SSD"))) {
...
/* set variables */
    ...
    $txtCapacity = '';
    $selCompatibleType = $specification[1];
...
/* click submit button */
    ...
    /* no errors */
        ...
        /* join specification */
            $specification = array($capacity, $selCompatibleType);
...
<body>
    ...
    <!-- add new items -->
        <div class="box-layout">
            ...
            <form action="/admin/edit-item/Drive/" method="post" enctype="multipart/form-data" autocomplete="off">
                <?php
            ...
            <!-- capacity -->
                ...
                <p class="custom-note">The drive capacity sizes you have previously added are listed here,
                    and if the drive capacity size you are adding appears in the list, you can select it. Alternatively, you can enter your new drive capacity size in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the drive capacity size,
                    it will be considered the drive capacity size of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selCapacity">
                        <option value="none">-- Select a previously entered name --</option>
                        <option value="Capacity: 128GB" <?php echo $selCapacity == "Capacity: 128GB" ? "selected" : '' ; ?>>128GB</option>
                        <option value="Capacity: 500GB" <?php echo $selCapacity == "Capacity: 500GB" ? "selected" : '' ; ?>>500GB</option>
                        <option value="Capacity: 1TB" <?php echo $selCapacity == "Capacity: 1TB" ? "selected" : '' ; ?>>1TB</option>
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE (`category` = 'HDD' OR `category` = 'SSD') AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                            $savedRecords = array("Capacity: 128GB", "Capacity: 500GB", "Capacity: 1TB");
                            $foundRecord = 0;
                            if ($stmt == true) {
                    ...
                    <p>Or</p>
                <!-- enter new capacity -->
                    <input type="text" name="txtCapacity" placeholder="Enter new Drive capacity (Ex: Capacity: 250GB)" value="<?php echo $txtCapacity; ?>" maxlength="20">
                </fieldset>
            ...
            <!-- compatible type -->
                ...
                <select name="selCompatibleType">
                    ...
                    <option value="Compatible Type: Laptop | Desktop" <?php echo $selCompatibleType == "Compatible Type: Laptop | Desktop" ? "selected" : '' ; ?>>Both (Laptop/ Desktop)</option>
                </select>
                <p class="custom-note">Select one of the above</p>
            <!-- description -->
                <h3 class="sub-header">Description (Optional)</h3>
                <textarea name="rtxtDescription" placeholder="Enter description (Ex: Writing Speed: 1Gbps, etc..)" maxlength="1500"><?php echo $rtxtDescription; ?></textarea>
