...
/* check if click delete button */
    if (isset($_GET['statusId']) && $_GET['delete'] == "yes") {
        $sql = "DELETE FROM `Status` WHERE `statusId` = '$_GET[statusId]'";
...
<head>
    <meta charset="UTF-8">
    <title>Dilla's Pc - Manage Status</title>
    ...
    /* search result styles */
        ...
        main .search-div table td img, main .search-div table td video {
            width: 100%;
            border: 1px solid #fff;
            border-radius: 4px;
        }
...
<body>
    ...
    <main>
        ...
        <h3 class="result-header">Manage Status 🛠️</h3>
        <div class="search-div">
        <!-- filter items -->
            <form action="/admin/manage-status/" method="get" autocomplete="off" class="search-form">
                <input type="search" name="search" value="<?php echo $search; ?>" placeholder="Type here anything uploaded status related for filter records.. 🔍" maxlength="100">
            </form>
            <table>
                <?php
                    ...
                    if ($stmt == true) {
                        while ($row = mysqli_fetch_assoc($stmt)) {
                            ...
                            if (substr($row['type'], 0, 5) == "image") {
                                echo "<img src='/status/$row[pathName]' alt='No status photo'>";
                            } else if (substr($row['type'], 0, 5) == "video") {
                                echo "<video controls controlsList='nodownload'><source src='/status/$row[pathName]'></video>";
                            }
                            echo "</td>";
                            echo "<td><a href='/admin/manage-status/?search=$row[description]'>$row[description]</a></td>";
                            echo "<td><a href='/admin/manage-status/?search=$row[duration]'>$row[duration]</a></td>";
                            echo "<td><a href='/admin/manage-status/?search=$row[lastUpdateDate]'>$row[lastUpdateDate]</a></td>";
                            echo "<td>";
                            ?><a href="/admin/manage-status/?statusId=<?php echo $row['statusId']; ?>&delete=yes&search=<?php echo $search; ?>"
                                onclick="return confirm('PERMANENTLY DELETE 🚮\n\n\tAre you sure you want to permanently delete the selected status?')"
