<?php
    ...
    if (!(isset($_REQUEST['category']) && strtoupper($_REQUEST['category']) == "RAM")) {
...
/* click submit button */
    ...
    /* set new values */
        ...
        $selCompatibleType = $_POST['selCompatibleType'];
    ...
    /* check errors */
        if ($selCapacity == "none" && strlen($txtCapacity) < 1) {
            $info[] = "RAM capacity (size) field is required. ⁉️"; # capacity field not fill error
        }
        if ($selMemoryType == "none" && strlen($txtMemoryType) < 1) {
            $info[] = "Memory type field is required. ⁉️"; # memory type field not fill error
        }
        if ($selCompatibleType == "none") {
            $info[] = "Please select compatible type of RAM. ⁉️"; # compatible type not selected error
        }
    ...
    /* no errors */
        if (empty($info)) {
            if ($txtCapacity > 0) {
                $selCapacity = "none"; # reset capacity list
                $capacity = mysqli_real_escape_string($conn, $txtCapacity);
            } else {
                $capacity = mysqli_real_escape_string($conn, $selCapacity);
            }
            if ($txtMemoryType > 0) {
                $selMemoryType = "none"; # reset memory type list
                $memoryType = mysqli_real_escape_string($conn, $txtMemoryType);
            } else {
                $memoryType = mysqli_real_escape_string($conn, $selMemoryType);
            }
        /* join specification */
            $specification = array($capacity, $memoryType, $selCompatibleType);
...
<body>
    ...
    <!-- add new items -->
        <div class="box-layout">
            ...
            <form action="/admin/add-item/RAM/" method="post" enctype="multipart/form-data" autocomplete="off">
                <?php
            ...
            <!-- capacity -->
                <h3 class="sub-header">Capacity (Size) *</h3>
                <p class="custom-note">The RAM capacities you have previously added are listed here,
                    and if the RAM capacity you are adding appears in the list, you can select it. Alternatively, you can enter your new RAM capacity in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the RAM capacity,
                    it will be considered the RAM capacity of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                    <select name="selCapacity">
                        ...
                        <?php
                            $sql = "SELECT `specification` FROM `Product` WHERE `category` = 'RAM' AND `trash` = 0";
                            $stmt = mysqli_query($conn, $sql);
                    ...
                    <p>Or</p>
                <!-- enter new capacity -->
                    <input type="text" name="txtCapacity" placeholder="Enter new RAM capacity (Ex: Capacity: 32GB)" value="<?php echo $txtCapacity; ?>" maxlength="20">
                </fieldset>
                <p class="custom-note">Please enter 1 to 20 characters</p>
            <!-- memory type -->
                <h3 class="sub-header">Memory Type *</h3>
                <p class="custom-note">The memory types you have previously added are listed here,
                    and if the memory type you are adding appears in the list, you can select it. Alternatively, you can enter your new memory type in the box below.</p>
                <p class="custom-note" style="margin-bottom: 20px;">Note: If you enter anything in the box corresponding to the memory type,
                    it will be considered the memory type of the new product you are entering.</p>
                <fieldset style="margin-bottom: 0;">
                ...
                <!-- enter new memory type -->
                    <input type="text" name="txtMemoryType" placeholder="Enter new RAM capacity (Ex: Memory Type: DDR5)" value="<?php echo $txtMemoryType; ?>" maxlength="15">
                </fieldset>
                <p class="custom-note">Please enter 1 to 15 characters</p>
            <!-- Compatible type -->
                <h3 class="sub-header">Compatible Type *</h3>
                <select name="selCompatibleType">
                    <option value="none">-- Not selected --</option>
                    <option value="Compatible Type: Laptop" <?php echo $selCompatibleType == "Compatible Type: Laptop" ? "selected" : '' ; ?>>Laptop</option>
                    <option value="Compatible Type: Desktop" <?php echo $selCompatibleType == "Compatible Type: Desktop" ? "selected" : '' ; ?>>Desktop</option>
                </select>
                <p class="custom-note">Please select one</p>
            <!-- description -->
                <h3 class="sub-header">Description (Optional)</h3>
                <textarea name="rtxtDescription" placeholder="Enter description (Ex: Bus Speed: 1400MHz, etc..)" maxlength="1500"><?php echo $rtxtDescription; ?></textarea>
