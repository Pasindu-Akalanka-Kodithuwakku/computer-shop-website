<?php
    ...
    try {
        ...
        header('Location: /product-category/computer-accessories/');
        exit();
    }
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Dilla's PC - Accessories Stock</title>
...
<body>
    ...
    <main>
        ...
        <div class="category-header" onclick="control_sidebar()"><span>&#9776; Computer Accessories </span><img src="/image/accessories.png" alt=""></div>
        <div class="filter-div" id="filter_sidebar">
            <form action="/product-category/computer-accessories/" method="post">
                <fieldset>
                    <details <?php echo isset($_REQUEST['category']) ? "open" : '' ; ?>>
                            ...
                            if ($subStmt == true) {
                                ...
                                while ($subRow = mysqli_fetch_assoc($subStmt)) {
                                    foreach ($category as $key => $value) {
                                        if ($value == $subRow['category']) {
                                            $foundData = $value;
                                            break;
                                        }
                                    }
                                    if ($foundData != '') {
                                        echo "<input type='checkbox' name='category[]' id='$subRow[category]' value='$subRow[category]' " .(($foundData == $subRow['category']) ? "checked" : '') ."> ";
                                    } else {
                                        echo "<input type='checkbox' name='category[]' id='$subRow[category]' value='$subRow[category]'> ";
                                    }
                                    echo "<label for='$subRow[category]'>$subRow[category]</label><br>";
                                }
                            }
                        ?>
                    </details>
                    ...
                    <div class="show-item" id="<?php echo $count; ?>">
                        ...
                        <div class="cart-item">
                            <?php
                                ...
                                ?><a href="/product-category/computer-accessories/?cart=<?php echo $row['productId'] .$getValues .'#' .$count; ?>">Add To Cart <img src="/image/cart.png" alt=""></a><?php # display add to cart option
                            ?>
                        </div>
                    </div>
                    <?php
